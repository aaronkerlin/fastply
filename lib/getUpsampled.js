...
function getUpsampled(trace, data) {
        var surface = trace.surface;
        var z = trace.data.z;
        var xlen = z[0].length;
        var ylen = z.length;
        var scaleF = trace.dataScale;
        var upsampled = ndarray(new Float32Array(xlen * ylen), [xlen, ylen]);

        if (data.shape) {
            fill(upsampled, function(row, col) {
